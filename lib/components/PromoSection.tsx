    ...
    if (navigator.share) {
      try {
        if (file && navigator.canShare && navigator.canShare({ files: [file] })) {
          await navigator.share({ files: [file], title: "Pawly 🐾", text });
        } else {
          await navigator.share({ title: "Pawly 🐾", text, url: "https://pawlyapp.ch" });
        }
        return;
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          try {
            await navigator.share({
              files: [file],
              title: "Pawly — Vidéo Promo 🐾",
              text: "Regarde la promo Pawly → https://pawlyapp.ch/promo",
            });
            setRecording(false);
            return;
          } catch {}
  ...
  const downloadVideo = useCallback(async () => {
    ...
    try {
        ...
        await new Promise<void>((resolve) => {
          ...
          img.onload = () => {
            ...
            if (slide.isHero) {
              ...
              const cta = "🐾 Rejoins Pawly — pawlyapp.ch";
      ...
      if (navigator.share) {
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          try {
            await navigator.share({ files: [file], title: "Pawly — Vidéo Promo 🐾", text: "Regarde la promo Pawly → https://pawlyapp.ch" });
            setRecording(false);
            setVideoProgress("");
            return;
          } catch {}
        ...
        {currentIsHero && (
          <div className="absolute bottom-0 left-0 right-0 z-20 p-3 flex flex-col gap-2" style={{ animation: "heroCTAAppear 0.8s ease-out" }}>
            ...
            <a
              ...
              style={{
                background: "linear-gradient(135deg, #22C55E 0%, #F43F5E 50%, #A78BFA 100%)",
                boxShadow: "0 4px 20px rgba(249,115,22,0.4)",
                animation: "ctaPulse 2s ease-in-out infinite",
              }}
            >
              🐾 Rejoins Pawly — C&apos;est gratuit !
            </a>
            {/* Replay button */}
            <button
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-3 gap-2 mb-2">
          <button
            ...
            style={{
              background: recording ? "#6b7280" : "linear-gradient(135deg, #22C55E, #EC4899)",
          ...
          <button
            onClick={() => shareWithImage("🐾 Pawly — Le Tinder des Animaux 🇨🇭\nRegarde la vidéo promo → https://pawlyapp.ch/promo\n\nConnecte ton compagnon avec d'autres animaux en Suisse ! Gratuit")}
            className="flex items-center justify-center gap-1.5 py-3 rounded-xl text-xs font-bold text-white transition-transform active:scale-95"
            style={{ background: "linear-gradient(135deg, #22C55E, #F43F5E)" }}
          >
            📤 Partager
          </button>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-2">
          ...
          <button
            onClick={async () => {
              ...
              alert("📸 Image téléchargée !\nOuvre Instagram → Story → choisis l'image Pawly depuis ta galerie.");
            }}
            className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-[11px] font-bold text-white transition-transform active:scale-95"
          ...
          <button
            onClick={async () => {
              await downloadPromoImage();
              alert("🎵 Image téléchargée !\nOuvre TikTok → + → choisis l'image Pawly depuis ta galerie.");
            }}
            className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-[11px] font-bold text-white transition-transform active:scale-95"
          ...
          <a
            href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent("https://pawlyapp.ch")}&quote=${encodeURIComponent("Pawly connecte les animaux en Suisse ! 🐾")}`}
          ...
          <a
            href={`https://x.com/intent/tweet?text=${encodeURIComponent("Pawly — Le Tinder des Animaux 🐾🇨🇭 Connecte ton compagnon →")}&url=${encodeURIComponent("https://pawlyapp.ch")}`}
            target="_blank" rel="noopener noreferrer"
            className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-[11px] font-bold text-white transition-transform active:scale-95"
            style={{ background: "#000" }}>
            ✖ X (Twitter)
          ...
          <button
            onClick={() => {
              navigator.clipboard.writeText("🐾 Pawly — Le Tinder des Animaux 🇨🇭\nGratuit → https://pawlyapp.ch").then(() => {
                setCopied(true);
                setTimeout(() => setCopied(false), 2000);
              }).catch(() => {});
            }}
