    ...
    <>
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex items-center justify-between h-12">
            ...
            <div className="md:hidden flex items-center gap-2">
              ...
              <div className="flex items-center gap-0.5">
                {LANGS.map((l) => (
                  <button key={l.code} onClick={() => setLang(l.code as any)}
                    className={"w-6 h-6 rounded-full text-[10px] transition-all flex items-center justify-center " +
                      (lang === l.code ? "bg-orange-500/20 border border-orange-500/50" : "opacity-40")}>
                    {l.flag}
                  </button>
                ))}
              </div>
              ...
              {!loading && !user && (
                <Link href="/signup" className="px-3 py-1 bg-orange-500 text-white text-[10px] font-bold rounded-full">
                  {t.navJoin}
                </Link>
              )}
            </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[var(--c-nav)]/95 backdrop-blur-xl border-t border-white/5 safe-area-bottom">
        <div className="flex items-center justify-around h-12 px-2">
          ...
          <BottomTab href="/carte" active={isActive("/carte")} label="Carte">
...
function NavLink({ href, active, label }: { href: string; active: boolean; label: string }) {
  return (
    <Link href={href} className={"px-4 py-2 rounded-full text-sm transition " +
