...
const PERSONALITIES: Record<string, Record<string, { name: string; emoji: string; color: string; desc: string }>> = {
  fr: {
    "Sociable": { name: "L'Ambassadeur", emoji: "🌟", color: "#FBBF24", desc: "Votre animal est un vrai social ! Il adore les rencontres et met tout le monde à l'aise." },
    ...
    "Calme": { name: "Le Zen", emoji: "🧘", color: "#FBBF24", desc: "La tranquillité incarnée. Votre animal apporte le calme partout où il va." },
    ...
    "Joueur": { name: "Le Farceur", emoji: "🎪", color: "#FBBF24", desc: "La vie est un jeu ! Votre animal transforme tout en moment fun." },
  },
  de: {
    "Sociable": { name: "Der Botschafter", emoji: "🌟", color: "#FBBF24", desc: "Dein Tier ist super gesellig! Es liebt Begegnungen und bringt alle zusammen." },
    "Energique": { name: "Der Tornado", emoji: "⚡", color: "#ef4444", desc: "Nicht zu bremsen! Dein Tier strotzt vor Energie und braucht Action." },
    "Calme": { name: "Der Zen-Meister", emoji: "🧘", color: "#FBBF24", desc: "Die Ruhe selbst. Dein Tier bringt überall Gelassenheit mit." },
    ...
    "Joueur": { name: "Der Spassvogel", emoji: "🎪", color: "#FBBF24", desc: "Das Leben ist ein Spiel! Dein Tier macht aus allem einen Spass." },
  },
  it: {
    "Sociable": { name: "L'Ambasciatore", emoji: "🌟", color: "#FBBF24", desc: "Il tuo animale è super socievole! Adora gli incontri e mette tutti a loro agio." },
    "Energique": { name: "Il Tornado", emoji: "⚡", color: "#ef4444", desc: "Impossibile fermarlo! Il tuo animale trabocca di energia." },
    "Calme": { name: "Lo Zen", emoji: "🧘", color: "#FBBF24", desc: "La tranquillità fatta animale. Porta calma ovunque vada." },
    ...
    "Joueur": { name: "Il Giocherellone", emoji: "🎪", color: "#FBBF24", desc: "La vita è un gioco! Trasforma tutto in divertimento." },
  },
  en: {
    "Sociable": { name: "The Ambassador", emoji: "🌟", color: "#FBBF24", desc: "Your pet is a true social butterfly! They love meetups and put everyone at ease." },
    "Energique": { name: "The Tornado", emoji: "⚡", color: "#ef4444", desc: "Can't keep up! Your pet is bursting with energy and needs action." },
    "Calme": { name: "The Zen Master", emoji: "🧘", color: "#FBBF24", desc: "Calm personified. Your pet brings peace wherever they go." },
    ...
    "Joueur": { name: "The Jester", emoji: "🎪", color: "#FBBF24", desc: "Life is a game! Your pet turns everything into a fun moment." },
  },
};
    ...
    return (
      <div style={{ background: "var(--c-card)", border: "2px solid var(--c-border)", borderRadius: 20, padding: 24, textAlign: "center" }}>
        ...
        <button onClick={() => setStarted(true)}
          style={{ padding: "12px 28px", background: "#FBBF24", color: "#fff", border: "none", borderRadius: 50, fontWeight: 800, fontSize: 14, cursor: "pointer", boxShadow: "0 0 20px rgba(249,115,22,0.3)" }}>
          {ui.startBtn}
        </button>
      </div>
    );
  ...
  return (
    <div style={{ background: "var(--c-card)", border: "2px solid var(--c-border)", borderRadius: 20, padding: 24 }}>
      <div style={{ display: "flex", gap: 4, marginBottom: 16 }}>
        {questions.map((_, i) => (
          <div key={i} style={{ flex: 1, height: 4, borderRadius: 2, background: i <= step ? "#FBBF24" : "var(--c-border)", transition: "background 0.3s" }} />
        ))}
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {q.answers.map((a, i) => (
          <button key={i} onClick={() => handleAnswer(a.traits)}
            ...
            onMouseOver={e => { (e.target as HTMLElement).style.borderColor = "#FBBF24"; (e.target as HTMLElement).style.background = "rgba(249,115,22,0.05)"; }}
