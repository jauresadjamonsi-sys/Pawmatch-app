    ...
    <div className="min-h-screen px-4 md:px-6 py-6 pb-28">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="text-3xl font-bold text-[var(--c-text)]">Ton prochain <span className="text-amber-400">pote</span></h1>
        </div>
        ...
        <div className="flex flex-wrap gap-3 mb-8">
          <input
            ...
            className="px-4 py-2 bg-[var(--c-card)] border border-[var(--c-border)] rounded-xl text-[var(--c-text)] placeholder-[var(--c-text-muted)] text-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none"
          />
          <select
            value={species}
            onChange={(e) => setSpecies(e.target.value)}
            className="px-4 py-2 bg-[var(--c-card)] border border-[var(--c-border)] rounded-xl text-[var(--c-text-muted)] text-sm focus:ring-2 focus:ring-amber-500 outline-none appearance-none"
          >
            {SPECIES_LIST.map((s) => (
              ...
              <Link href={"/animals/" + animal.id} key={animal.id} className="bg-[var(--c-card)] border border-[var(--c-border)] rounded-2xl overflow-hidden hover:bg-[var(--c-card)] transition group">
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-2">
                    {animal.canton && (
                      <span className="text-[10px] px-2 py-0.5 bg-amber-500/20 text-amber-300 rounded-full">{animal.canton}</span>
                    )}
                    {animal.city && (
