          ...
          <Marker key={a.id} position={[coords[0] + off[0], coords[1] + off[1]]}>
            <Popup>
              <div style={{ textAlign: "center", minWidth: 120 }}>
                ...
                <a href={"/animals/" + a.id} style={{ fontSize: 11, color: "#FBBF24", fontWeight: 700 }}>{t.mapSee}</a>
              </div>
            </Popup>
          </Marker>
        );
...
export default function CartePage() {
  ...
  useEffect(() => {
    async function load() {
      let q = supabase.from("animals").select("id, name, species, photo_url, city, canton, breed").limit(500);
          ...
          {[{ v: "", l: t.mapAll, e: "🐾" }, { v: "chien", l: t.mapDogs, e: "🐕" }, { v: "chat", l: t.mapCats, e: "🐱" }, { v: "lapin", l: t.mapRabbits, e: "🐰" }].map(s => (
            <button key={s.v} onClick={() => setFilter(s.v)}
              style={{ padding: "4px 10px", borderRadius: 50, border: "1px solid var(--c-border)", cursor: "pointer", fontSize: 10, fontWeight: 700,
                background: filter === s.v ? "#FBBF24" : "var(--c-card)", color: filter === s.v ? "#fff" : "var(--c-text)" }}>
              {s.e} {s.l}
            </button>
          ))}
        </div>
