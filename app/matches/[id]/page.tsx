...
export default function ConversationPage() {
    ...
    return (
      <div className="min-h-screen bg-[var(--c-deep)] flex items-center justify-center">
        <div className="bg-[var(--c-card)] border border-[var(--c-border)] rounded-2xl p-5 md:p-8 text-center w-full max-w-sm md:max-w-md mx-4">
          ...
          <Link href="/matches" className="inline-block mt-4 text-amber-400 font-medium">← Retour</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-[100dvh] bg-[var(--c-deep)] flex flex-col">
      ...
      <div className="backdrop-blur-xl border-b border-[var(--c-border)] px-4 py-3 sticky top-0 z-10" style={{ background: "var(--c-deep)" }}>
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          ...
          <Link href={"/animals/" + theirAnimal.id} className="w-9 h-9 rounded-full bg-[var(--c-card)] border border-amber-400/30 flex items-center justify-center overflow-hidden flex-shrink-0 relative">
          ...
          <Link href={"/animals/" + theirAnimal.id} className="flex-1 min-w-0 no-underline">
            <p className="font-semibold text-[var(--c-text)] text-sm truncate hover:text-amber-300 transition">
              {otherProfile.full_name || otherProfile.email}
            </p>
            <p className="text-xs text-[var(--c-text-muted)]">
              {theirAnimal.name} x {myAnimal.name}
              {isOtherTyping && (
                <span className="ml-1.5 text-[var(--c-accent,#FBBF24)]">
          ...
          <Link
            ...
            className="p-1.5 rounded-full hover:bg-amber-400/10 transition flex-shrink-0"
            title="Appel video"
          >
            <svg className="w-4.5 h-4.5 text-[var(--c-accent,#FBBF24)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {/* Reconnection banner */}
      {reconnecting && (
        <div className="reconnect-banner bg-[var(--c-accent,#FBBF24)]/10 border-b border-[var(--c-accent,#FBBF24)]/20 px-4 py-1.5 text-center">
          <p className="text-xs text-[var(--c-accent,#FBBF24)] font-medium">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        <div className="max-w-3xl mx-auto space-y-2">
          ...
          <div className="glass rounded-2xl p-4 mb-4 text-center" style={{ borderLeft: "3px solid var(--c-accent, #FBBF24)" }}>
            <p className="text-sm font-semibold text-[var(--c-text)] mb-2">{"🐾"} Charte de bienveillance PawlyApp</p>
                  ...
                  <div className="relative max-w-[85%] sm:max-w-[70%]">
                    ...
                    <div
                      className={"rounded-2xl text-sm select-none " +
                        (isMine
                          ? "bg-gradient-to-br from-amber-400 to-amber-500 text-white rounded-br-md"
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            ...
            {suggestions.map((s, i) => (
              <button
                ...
                disabled={sending}
                className="suggestion-chip flex-shrink-0 px-3 py-1.5 bg-[var(--c-card)] border border-[var(--c-border)] hover:border-amber-400/40 hover:bg-amber-400/10 text-[var(--c-text-muted)] hover:text-[var(--c-text)] rounded-full text-xs transition-all duration-150 active:scale-95 whitespace-nowrap"
                style={{ animationDelay: i * 0.05 + "s" }}>
                {aiReady && i === 0 && <span className="mr-1 text-amber-300">✦</span>}
                {s}
              </button>
            ))}
          </div>
            ...
            {isRecordingVoice ? (
              ...
              <>
                ...
                <button
                  ...
                  className="w-10 h-10 bg-[var(--c-card)] border border-[var(--c-border)] hover:border-amber-400/30 hover:bg-amber-400/10 rounded-2xl flex items-center justify-center transition active:scale-95 flex-shrink-0"
                ...
                <div className="relative">
                  <button
                    ...
                    className={"w-10 h-10 border rounded-2xl flex items-center justify-center transition active:scale-95 flex-shrink-0 " + (showEmojiPicker ? "bg-amber-400/10 border-amber-400/40" : "bg-[var(--c-card)] border-[var(--c-border)] hover:border-amber-400/30 hover:bg-amber-400/10")}
                ...
                <input
                  ...
                  placeholder="Message..."
                  maxLength={2000}
                  className="flex-1 px-4 py-2.5 bg-[var(--c-card)] border border-[var(--c-border)] rounded-2xl text-[var(--c-text)] placeholder-[var(--c-text-muted)] focus:ring-1 focus:ring-amber-400/50 focus:border-amber-400/30 outline-none text-sm transition"
                />
                ...
                {!newMessage.trim() ? (
                  ...
                  <button
                    ...
                    className="w-10 h-10 bg-amber-400 hover:bg-amber-500 disabled:opacity-30 rounded-2xl flex items-center justify-center transition active:scale-95 flex-shrink-0">
                    {sending
